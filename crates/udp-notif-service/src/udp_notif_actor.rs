...
//!
//! 1. **Command Channel**: Receives `UdpNotifActorCommand` messages to control
...
//!    [UdpNotifActorHandle::new()] is called.
...
//!
//! ```rust,no_run
//! use netgauze_udp_notif_service::udp_notif_actor::UdpNotifActorHandle;
...
//!         UdpNotifActorHandle::new(1, addr, 100, Duration::from_millis(500))
//!             .await
...
pub(crate) enum UdpNotifActorCommand {
...
struct UdpNotifActor {
    ...
    cmd_rx: mpsc::Receiver<UdpNotifActorCommand>,
    ...
    clients: HashMap<SocketAddr, UdpNotifCodec>,
}
...
impl UdpNotifActor {
    ...
    pub fn decode_pkt(&mut self, next: (BytesMut, SocketAddr)) -> Option<UdpNotifRequest> {
        ...
        // If we haven't seen the client before, create a new UdpNotifCodec for it.
        // UdpNotifCodec handles the decoding/encoding of packets.
    ...
    /// This function handles a decoded udp-notif message.
    /// In the current implementation, it sends the message to all subscribers.
    async fn handle_decoded_msg(&mut self, next: Option<UdpNotifRequest>) {
        if let Some((peer, msg)) = next {
            ...
            let flow_request = Arc::new((peer, msg));
    ...
    async fn handle_subscribe(
        ...
        info!(
            "[Actor {}-{}] new subscriber {} is registered",
            self.actor_id, self.socket_addr, id
        );
        if let Err(_err) = tx
            .send(Subscription {
            ...
            .await
        {
            ...
            warn!(
                "[Actor {}-{}] new subscriber {} is removed, unable to send back the subscriber id",
                self.actor_id, self.socket_addr, id
            );
        }
        false
    ...
    async fn handle_purge_unused_peers(
        ...
        self.peers_usage.retain(|peer, usage| {
            ...
            if since > dur && usage.current_count == 0 {
                info!(
                    "[Actor {}-{}] removing peers that have not been used for the past {:?}",
                    self.actor_id, self.socket_addr, dur
                );
    ...
    async fn handle_cmd(
        &mut self,
        cmd: Option<UdpNotifActorCommand>,
    ) -> Result<bool, UdpNotifActorError> {
        let cmd_result = match cmd {
            Some(UdpNotifActorCommand::Shutdown(tx)) => self.handle_shutdown(tx).await,
            Some(UdpNotifActorCommand::Subscribe(tx, msg_tx)) => {
                self.handle_subscribe(tx, msg_tx).await
            }
            Some(UdpNotifActorCommand::Unsubscribe(id, tx)) => {
                self.handle_unsubscribe(id, tx).await
            }
            Some(UdpNotifActorCommand::PurgePeer(peer, tx)) => {
                self.handle_purge_peer(peer, tx).await
            }
            Some(UdpNotifActorCommand::PurgeUnusedPeers(dur, ret_tx)) => {
                self.handle_purge_unused_peers(dur, ret_tx).await
            }
            Some(UdpNotifActorCommand::LocalAddr(tx)) => self.handle_local_addr(tx).await,
            Some(UdpNotifActorCommand::GetPeers(tx)) => self.handle_get_peers(tx).await,
            None => {
                warn!(
...
#[derive(Debug)]
pub enum UdpNotifActorHandleError {
    SendError,
    ReceiveError,
}

impl std::fmt::Display for UdpNotifActorHandleError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match *self {
            UdpNotifActorHandleError::SendError => {
                write!(f, "Error sending command to actor")
            }
            UdpNotifActorHandleError::ReceiveError => {
...
impl std::error::Error for UdpNotifActorHandleError {
    fn description(&self) -> &str {
        match *self {
            UdpNotifActorHandleError::SendError => "Error sending command to actor",
            UdpNotifActorHandleError::ReceiveError => "Error receiving response from actor",
        }
    }
...
/// actor.
#[derive(Debug, Clone)]
pub struct UdpNotifActorHandle {
    actor_id: ActorId,
    local_addr: SocketAddr,
    cmd_buffer_size: usize,
    pub(crate) cmd_tx: mpsc::Sender<UdpNotifActorCommand>,
}

impl UdpNotifActorHandle {
    pub async fn new(
        actor_id: ActorId,
        socket_addr: SocketAddr,
        cmd_buffer_size: usize,
    ...
    > {
        ...
        cmd_tx
            .send(UdpNotifActorCommand::LocalAddr(tx))
            .await
            .map_err(|_| UdpNotifActorHandleError::SendError)?;
        let local_addr = rx
            .recv()
            .await
            .ok_or(UdpNotifActorHandleError::ReceiveError)?
            .1;
        Ok((
            join_handle,
            Self {
    ...
    pub async fn subscribe(
        &self,
        buffer_size: usize,
    ) -> Result<(UdpNotifReceiver, Vec<Subscription>), UdpNotifActorHandleError> {
    ...
    pub async fn subscribe_tx(
        &self,
        msg_tx: UdpNotifSender,
    ) -> Result<Vec<Subscription>, UdpNotifActorHandleError> {
        let (tx, mut rx) = mpsc::channel(self.cmd_buffer_size);
        self.cmd_tx
            .send(UdpNotifActorCommand::Subscribe(tx, msg_tx))
            .await
            .map_err(|_| UdpNotifActorHandleError::SendError)?;
    ...
    pub async fn unsubscribe(
        ...
        let (tx, mut rx) = mpsc::channel(self.cmd_buffer_size);
        self.cmd_tx
            .send(UdpNotifActorCommand::Unsubscribe(subscription, tx))
            .await
            .map_err(|_| UdpNotifActorHandleError::SendError)?;
        let mut subscriptions = vec![];
        while let Some(subscription) = rx.recv().await {
    ...
    pub async fn shutdown(&self) -> Result<Vec<ActorId>, UdpNotifActorHandleError> {
        let (tx, mut rx) = mpsc::channel(self.cmd_buffer_size);
        self.cmd_tx
            .send(UdpNotifActorCommand::Shutdown(tx))
            .await
            .map_err(|_| UdpNotifActorHandleError::SendError)?;
        let mut actors = vec![];
    ...
    pub async fn purge_unused_peers(
        &self,
        duration: Duration,
    ) -> Result<Vec<SocketAddr>, UdpNotifActorHandleError> {
        let (tx, mut rx) = mpsc::channel(self.cmd_buffer_size);
        // If the command fails, the recv after will fail, no need to double handle the
        // error
        self.cmd_tx
            .send(UdpNotifActorCommand::PurgeUnusedPeers(duration, tx))
            .await
            .map_err(|_| UdpNotifActorHandleError::SendError)?;
        let mut peers = vec![];
    ...
    pub async fn purge_peer(
        &self,
        peer: SocketAddr,
    ) -> Result<Option<ActorId>, UdpNotifActorHandleError> {
        let (tx, mut rx) = mpsc::channel(self.cmd_buffer_size);
        self.cmd_tx
            .send(UdpNotifActorCommand::PurgePeer(peer, tx))
            .await
            .map_err(|_| UdpNotifActorHandleError::SendError)?;
        Ok(rx.recv().await.flatten())
    }

    pub async fn get_peers(&self) -> Result<(ActorId, Vec<SocketAddr>), UdpNotifActorHandleError> {
        let (tx, mut rx) = mpsc::channel(self.cmd_buffer_size);
        self.cmd_tx
            .send(UdpNotifActorCommand::GetPeers(tx))
            .await
            .map_err(|_| UdpNotifActorHandleError::SendError)?;
...
mod tests {
    ...
    use netgauze_udp_notif_pkt::{MediaType, UdpNotifMsg};
    ...
    async fn setup_actor() -> (
        ...
        let (join_handle, handle) =
            UdpNotifActorHandle::new(actor_id, socket_addr, 10, Duration::from_secs(1))
                .await
    ...
    /*
    fn generate_udp_notif_pkg(payload: Bytes) -> UdpNotifPacket {
        // Create a udp-notif pkt with some test data
        let pkt = UdpNotifPacket::new(
            UdpNotifHeader::new(
                ...
                //<usize as TryInto<u64>>::try_into(payload.len().try_into().expect("Failed to convert payload length")) + 12,
                1,
                1,
                HashMap::new(),
            ),
    ...
    fn generate_udp_notif_data(payload: Bytes) -> BytesMut {
        ...
        //let pkt = generate_udp_notif_pkg(payload);
        //pkt.write(&mut cur).expect("Failed to serialize the udp-notif packet");
        //pkt.serialize(&mut cur).expect("Failed to serialize the udp-notif packet");
        //pkt.serialize!(&mut cur).expect("Failed to serialize the udp-notif packet");
        //ser::expand_derive_serialize(&mut cur)
        //    .unwrap_or_else("Failed to serialize the udp-notif packet".to_string())
        //    .into();
        //pkt.into()
        let msg = generate_udp_notif_msg(payload);
        let mut codec = UdpNotifCodec::default();
        codec.encode(msg, &mut buf).expect("Failed to serialize the udp-notif packet");
        buf.into()
    }
    */
    ...
    fn generate_udp_notif_data(payload: Bytes) -> BytesMut {
        // Create udp-notif test data
        let msg = UdpNotifMsg::new(false, MediaType::YangDataJson, 1_u32, 1_u32, payload);
    ...
    async fn test_purge_unused_peers() {
        ...
        let mut data = generate_udp_notif_data(Bytes::from_static(b"test data"));
        tokio::task::yield_now().await;
    ...
    /*
        ...
        async fn test_purge_peer() {
            ...
            let data = generate_udp_notif_data(test_data);
            send_data(listening_socket, &socket, &mut data).await;
            ...
            assert_eq!(
                handled_pkt,
                Ok(Arc::new((socket.local_addr().unwrap(), test_data)))
            );
    ...
    */
    ...
    /*
        ...
        async fn test_receive_udp_packet() {
            ...
            // Generate and send udp-notif data to register the peer
            let test_data = Bytes::from_static(b"test data");
            let mut data = generate_udp_notif_data(test_data);
            send_data(listening_socket, &socket, &mut data).await;
    ...
    */
    ...
    /*
        ...
        async fn test_concurrent_behavior() {
            ...
            let test_data1 = Bytes::from_static(b"test data 1");
            let test_data2 = Bytes::from_static(b"test data 2");
            let mut data1 = generate_udp_notif_data(test_data1);
            let mut data2 = generate_udp_notif_data(test_data2);
            ...
            assert_eq!(received1, Ok(Ok(Arc::new((local_addr1, test_data1)))));
            assert_eq!(received2, Ok(Ok(Arc::new((local_addr2, test_data2)))));
        }
    */
    ...
    */
    ...
    async fn test_send_command_to_closed_actor() {
        ...
        assert!(matches!(result, Err(UdpNotifActorHandleError::SendError)));
    }
}
