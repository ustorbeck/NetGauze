...
//!
//! This module implements a supervisor that manages multiple
//! `UdpNotifActor` instances. It utilizes the actor model to provide a
//! scalable and fault-tolerant system for collecting and processing udp-notif
//! data.
//!
...
//!
//! The main supervisor that manages multiple `UdpNotifActor`
...
//!
//! Provides a public interface for interacting with the supervisor. It
...
use crate::{
    ...
    udp_notif_actor::{UdpNotifActorCommand, UdpNotifActorError, UdpNotifActorHandle},
...
enum UdpNotifSupervisorCommand {
    /// Command to send to all [UdpNotifActor]
    UdpNotifActorCommand(UdpNotifActorCommand),
...
struct UdpNotifSupervisor {
    actor_handlers: HashMap<ActorId, UdpNotifActorHandle>,
}

impl UdpNotifSupervisor {
    fn new(actor_handlers: HashMap<ActorId, UdpNotifActorHandle>) -> Self {
        Self { actor_handlers }
    }
    ...
    /// Handles [UdpNotifSupervisorCommand]
    async fn cmd(&mut self, mut rx: mpsc::Receiver<UdpNotifSupervisorCommand>) -> Result<(), ()> {
        while let Some(cmd) = rx.recv().await {
            match cmd {
                UdpNotifSupervisorCommand::UdpNotifActorCommand(flow_cmd) => {
                    debug!(
                        "[Supervisor] broadcasting to {} actors the udp-notif command: {}",
                ...
                UdpNotifSupervisorCommand::Unsubscribe(subscriptions, tx) => {
                    ...
                    for (actor_id, handle) in &self.actor_handlers {
                        ...
                        if let Err(err) = tx.send(send_back).await {
                            error!(
                                "[Supervisor] failed to send back the results unsubscribe command from Actor {}-{}: {:?}",
                                handle.actor_id(),
                                handle.local_addr(),
                                err
                            )
                ...
                UdpNotifSupervisorCommand::Shutdown(tx) => {
    ...
    async fn run(
        mut self,
        join_handles: Vec<JoinHandle<Result<(ActorId, SocketAddr), UdpNotifActorError>>>,
        rx: mpsc::Receiver<UdpNotifSupervisorCommand>,
    ) {
        tokio::select! {
            biased;
            _ = self.cmd(rx) => {
...
pub struct UdpNotifSupervisorHandle {
    cmd_tx: mpsc::Sender<UdpNotifSupervisorCommand>,
...
impl UdpNotifSupervisorHandle {
    pub async fn new(config: SupervisorConfig) -> (JoinHandle<()>, UdpNotifSupervisorHandle) {
        ...
        for binding_address in config.binding_addresses {
            for _ in 0..binding_address.num_workers {
                ...
                let actor_ret = UdpNotifActorHandle::new(
                    next_actor_id,
    ...
    pub async fn shutdown(&self) -> Result<(), UdpNotifSupervisorHandleError> {
        trace!("[Supervisor] sending shutting down message to supervisor");
        let (tx, rx) = oneshot::channel();
        if let Err(err) = self
            .cmd_tx
            .send(UdpNotifSupervisorCommand::Shutdown(tx))
            .await
        {
    ...
    pub async fn subscribe(
        ...
        trace!("[Supervisor] sending new subscription request to supervisor");
    ...
    pub async fn subscribe_tx(
        ...
        trace!(
            "[Supervisor] sending new subscription with pre-created channel request to supervisor"
        );
        let (tx, mut rx) = mpsc::channel(self.cmd_buffer_size);
        if let Err(err) = self
            .cmd_tx
            .send(UdpNotifSupervisorCommand::UdpNotifActorCommand(
                UdpNotifActorCommand::Subscribe(tx, pkt_tx),
            ))
            .await
        {
            error!("[Supervisor] error sending subscription request: {:?}", err);
            return Err(UdpNotifSupervisorHandleError::SendError);
        }
    ...
    pub async fn unsubscribe(
        ...
        if let Err(err) = self
            .cmd_tx
            .send(UdpNotifSupervisorCommand::Unsubscribe(subscriptions, tx))
            .await
        {
            error!(
                "[Supervisor] error sending unsubscription request: {:?}",
                err
            );
            return Err(UdpNotifSupervisorHandleError::SendError);
        }
    ...
    pub async fn purge_unused_peers(
        ...
        trace!("[Supervisor] sending new purge unused peers request to supervisor");
        let (tx, mut rx) = mpsc::channel(self.cmd_buffer_size);
        if let Err(err) = self
            .cmd_tx
            .send(UdpNotifSupervisorCommand::UdpNotifActorCommand(
                UdpNotifActorCommand::PurgeUnusedPeers(duration, tx),
            ))
            .await
        {
            error!(
    ...
    pub async fn purge_peer(
        ...
        trace!(
            "[Supervisor] sending new purge {} request to supervisor",
            peer
        );
        let (tx, mut rx) = mpsc::channel(self.cmd_buffer_size);
        if let Err(err) = self
            .cmd_tx
            .send(UdpNotifSupervisorCommand::UdpNotifActorCommand(
                UdpNotifActorCommand::PurgePeer(peer, tx),
            ))
            .await
        {
            error!(
    ...
    pub async fn local_addresses(
        ...
        if let Err(err) = self
            .cmd_tx
            .send(UdpNotifSupervisorCommand::UdpNotifActorCommand(
                UdpNotifActorCommand::LocalAddr(tx),
            ))
            .await
        {
            error!(
    ...
    pub async fn get_peers(
        ...
        if let Err(err) = self
            .cmd_tx
            .send(UdpNotifSupervisorCommand::UdpNotifActorCommand(
                UdpNotifActorCommand::GetPeers(tx),
            ))
            .await
        {
...
mod test {
    ...
    use netgauze_udp_notif_pkt::{codec::UdpNotifCodec, MediaType, UdpNotifMsg};
    use tokio::{
        net::UdpSocket,
